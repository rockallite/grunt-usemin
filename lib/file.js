...
var getBlocks = function (content) {
  ...
  //   * 3 : the output
  //
  var htmlregbuild = /<!--\s*build:(\w+)(?:\(([^\)]+)\))?\s*(.+?)\s*-->/;
  // end build pattern -- <!-- endbuild -->
  var htmlregend = /<!--\s*endbuild\s*-->/;
  ...
  //   * 3 : the output
  //
  var jsregbuild = /\/\*\*\*\s*build:(\w+)(?:\(([^\)]+)\))?\s*(.+?)\s*\*\*\*\//;
  // end build pattern -- /*** endbuild ***/
  var jsregend = /\/\*\*\*\s*endbuild\s*\*\*\*\//;
  ...
  lines.forEach(function (l) {
    var htmlbuild, jsbuild, htmlendbuild, jsendbuild, indent, asset;
    var build = (htmlbuild = l.match(htmlregbuild)) || (jsbuild = l.match(jsregbuild));
    var endbuild = (htmlendbuild = htmlregend.test(l)) || (jsendbuild = jsregend.test(l));
