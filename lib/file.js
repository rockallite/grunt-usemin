...
var getBlocks = function (content) {
  ...
  lines.forEach(function (l) {
    ...
    if (block && endbuild) {
      last.raw.push(l);
      // Allow empty output target for single source file only
      if (!last.dest) {
        var srcLen = last.src.length;
        if (srcLen > 1) {
          throw new Error('Empty output target for multiple source files not allowed:\n' + last.raw.join('\n'));
        }
        if (srcLen === 0) {
          throw new Error('Empty output target for empty source files not allowed:\n' + last.raw.join('\n'));
        }
        last.dest = last.src[0];
      }
      sections.push(last);
      block = false;
    ...
    if (block && last) {
      if (!exclusion && !build) {
        // 1. Skip <!-- exclude --> or /*** exclude ***/ sections
        // 2. Skip matching the <!-- build --> or /*** build ***/ lines for source files
